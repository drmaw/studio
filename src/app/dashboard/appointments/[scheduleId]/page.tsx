...
export default function DoctorAppointmentsPage() {
    ...
    // We need to find which organization this schedule belongs to.
    const orgId = appointments?.[0]?.organizationId;

    const { data: schedule, isLoading: scheduleLoading } = useDoc<DoctorSchedule>(
        useMemoFirebase(() => {
            if (!firestore || !orgId || !scheduleId) return null;
            return doc(firestore, 'organizations', orgId, 'schedules', scheduleId);
