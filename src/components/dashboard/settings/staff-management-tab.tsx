...
const initialStaff: User[] = [
    allUsers.find(u => u.id === 'user-doc-1')!,
];
...
export function StaffManagementTab() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // For the mock, we'll use email as the health ID for simplicity
    const userToAdd = allUsers.find(u => u.email.startsWith(values.healthId));
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col sm:flex-row items-end gap-4 p-4 border rounded-lg">
                    ...
                    render={({ field }) => (
                        ...
                        <FormControl>
                            <Input placeholder="Enter user's Health ID (e.g., doctor2)" {...field} />
                        </FormControl>
                        <FormMessage />
                        </FormItem>
                    )}
