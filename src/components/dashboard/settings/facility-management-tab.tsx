...
import { Loader2, PlusCircle, Hospital, Pencil, Trash2 } from "lucide-react";
                ...
                facilities.map((facility) => (
                  <TableRow key={facility.id}>
                    ...
                    <TableCell className="text-right space-x-1">
                      ...
                      <ConfirmationDialog
                            trigger={
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive/10 hover:text-destructive" disabled={deletingId === facility.id}>
                                     {deletingId === facility.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                                </Button>
                            }
